...
matrix2 = "np SSLM from Chromas euclidean 2pool3"
epochs = "200" #load model trained this number of epochs
#Model loading
...
model.load_state_dict(torch.load("/media/carlos/FILES/INVESTIGACIÓN/Proyectos/Boundaries Detection/Trained models/MLS_SSLM_MFCCs_eucl_Chromas_eucl_2pool3/saved_model_" + epochs + "epochs.bin", map_location=torch.device('cpu')))
model.eval()
...
im_path_L_MFCCs = "/media/carlos/FILES/INVESTIGACIÓN/Proyectos/Boundaries Detection/Inputs/TEST/" + matrix + "/"
im_path_L_MFCCs2 = "/media/carlos/FILES/INVESTIGACIÓN/Proyectos/Boundaries Detection/Inputs/TEST/" + matrix2 + "/"
...
for i in range(len(mels_trainloader)):
    ...
    image_sslm = torch.cat((image_sslm, image_sslm2), 1)
    ...
    #peaks_positions = np.insert(peaks_positions, 0, 0)
...
print("Input: MLS +", matrix, "+", matrix2)
print("Epochs:", epochs)
