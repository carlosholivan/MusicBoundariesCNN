...
im_path_mel_near_train = "E:\\INVESTIGACIÓN\\Proyectos\\Boundaries Detection\\Inputs\\TRAIN\\np MLS\\"
im_path_L_MFCCs_near_train = "E:\\INVESTIGACIÓN\\Proyectos\\Boundaries Detection\\Inputs\\TRAIN\\np SSLM from MFCCs euclidean 2pool3\\"
im_path_mel_near_val = "E:\\INVESTIGACIÓN\\Proyectos\\Boundaries Detection\\Inputs\\VALIDATION\\np MLS\\"
im_path_L_MFCCs_near_val = "E:\\INVESTIGACIÓN\\Proyectos\\Boundaries Detection\\Inputs\\VALIDATION\\np SSLM from MFCCs euclidean 2pool3\\"
...
writer_train = SummaryWriter("E:\\INVESTIGACIÓN\\Proyectos\\Boundaries Detection\\graphs\\train")
writer_test = SummaryWriter("E:\\INVESTIGACIÓN\\Proyectos\\Boundaries Detection\\graphs\\validation")
...
for epoch in range(num_epochs):
    ...
    "==============================TRAIN LOOP================================="
    optimizer.zero_grad()
    for batch_idx, ((images_mels, labels_mels, labels_sec_mels), (images_sslms, labels_sslms, labels_sec_sslms)) in enumerate(zip(trainloader_mels,trainloader_sslms)):
    ...
    with torch.no_grad(): #para que no calcule gradientes en validación
        for batch_idx, ((images_mels, labels_mels, labels_sec_mels), (images_sslms, labels_sslms, labels_sec_sslms)) in enumerate(zip(valloader_mels,valloader_sslms)):
            #Forward pass
            if images_mels.shape[3] != images_sslms.shape[3]:
                images_mels = images_mels[:,:,:,1:]
                labels_mels = labels_mels[:,1:]
