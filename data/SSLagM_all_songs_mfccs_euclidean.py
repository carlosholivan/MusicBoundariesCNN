...
"=================================Paths====================================="
song_path = "/media/carlos/FILES1/SALAMI/songs_separated_sources/demucs/"
im_path_L_MFCCs_near = "/media/carlos/FILES1/INVESTIGACIÓN/Proyectos/BoundariesDetectionSeparationSources/INPUTS/SSLM_MFCCs_euclidean/"
...
for root, dirs, files in os.walk(song_path):
    for song in os.listdir(song_path):
        for name_song in files:
            ...
            print("song", song, song_id, "prepared to be processed.")
            if not os.path.exists(im_path_L_MFCCs_near + song + "/"):
                os.makedirs(im_path_L_MFCCs_near + song + "/")
            if (song not in os.listdir(im_path_L_MFCCs_near)) or (song  + "_" + str(song_id) + ".npy" not in os.listdir(im_path_L_MFCCs_near + song + "/")):
                
                y, sr = librosa.load(song_path + song + "/" + name_song, sr=None)
                mel = mel_spectrogram(sr_desired, song_path + song + "/" + name_song, window_size, hop_length)
                sslm_near = sslm(mel, p, L_near)[0]
                
                if sslm_near.shape[1] < max_pooling(mel, 6).shape[1]:
                    sslm_near = np.hstack((np.ones((301,1)),sslm))
                elif sslm.shape[1] > max_pooling(mel, 6).shape[1]:
                    sslm_near = sslm_near[:,1:]
                
                #Save mels matrices and sslms as numpy arrays in separate paths 
                np.save(im_path_L_MFCCs_near + song + "/" + song + "_" + song_id, sslm_near)
                        
                print("song:", song, song_id, "converted.",  i,"/", len(os.listdir(song_path))*4, "song converted to all files in : {:.2f}s".format(time.time() - start_time_song))
