...
dataset_sslms_train = SSMDataset(im_path_L_MFCCs_near_train, labels_path, transforms=[normalize_image, padding_SSLM, borders])
dataset_sslms_val = SSMDataset(im_path_L_MFCCs_near_val, labels_path, transforms=[normalize_image, padding_SSLM, borders])
...
for epoch in range(num_epochs):
    ...
    examples = 0
    pbar = tqdm(total = len(trainloader_sslms))
    print("Epoch:", epoch)

    "==============================TRAIN LOOP================================="
    optimizer.zero_grad()
    for batch_idx, (images_sslms, labels_sslms, labels_sec_sslms) in enumerate(trainloader_sslms):
        #train in first convolution MLS and SSLMs separately

        images_sslms = images_sslms.to(device)

        outputs_combined = model(images_sslms.float())
            
        outputs = outputs_combined.view(-1) #2º valor son las clases de salida
        labels = labels_sslms.view(-1).float().to(device) *0.98 + 0.01
        ...
        labels_sec_sslms = labels_sec_sslms[0,:]
        reference = np.array((np.copy(labels_sec_sslms[:-1]), np.copy(labels_sec_sslms[1:]))).T
        repeated_list = []
    ...
    writer_train.add_scalar('Loss', training_loss / len(trainloader_sslms), epoch)
    writer_train.add_scalar('Accuracy', training_accuracy / len(trainloader_sslms), epoch)
    
    
    n_songs_train = len(trainloader_sslms)
    ...
    with torch.no_grad(): #para que no calcule gradientes en validación
        for batch_idx, (images_sslms, labels_sslms, labels_sec_sslms) in enumerate(valloader_sslms):
            #Forward pass
            images_sslms = images_sslms.to(device)

            val_outputs_combined = model(images_sslms.float())
        ...
        print("validation_accuracy={:.2f}  validation_loss={:.2f}\n".format(validation_accuracy / len(valloader_sslms), val_loss / len(valloader_sslms)))

        writer_test.add_scalar('Loss', val_loss / len(valloader_sslms), epoch) #epoch* len(valloader) + batch_idx
        writer_test.add_scalar('Accuracy', validation_accuracy / len(valloader_sslms), epoch)
        
        n_songs = len(valloader_sslms)
