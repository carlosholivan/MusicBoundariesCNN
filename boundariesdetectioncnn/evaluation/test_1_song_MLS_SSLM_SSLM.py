...
matrix = "np SSLM from MFCCs cosine 2pool3"
matrix2 = "np SSLM from Chromas cosine 2pool3"
...
sslm2 = np.expand_dims(sslms_dataset2[k][0], 0)
sslm2 = torch.Tensor(sslm2)

sslm_combined = torch.cat((sslm, sslm2), 1)

pred = model(mel, sslm_combined)
...
plt.plot(vector, sslms_dataset2[k][1]*300, 'r-', label='Labels')
plt.plot(vector, pred[:,0].detach().numpy()*300, 'w-', label='Output')
plt.imshow(sslms_dataset2[k][0][0,...], origin = 'lower', aspect=1)
plt.ylabel("lag bins")
